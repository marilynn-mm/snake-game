...
class SpampedeModel {
	...
	private void updateTail() {
		...
		snakeCells.remove(getSnakeTail());
	}
	
	private void updateHead(BoardCell nextCell) {
		getSnakeHead().becomeBody();
		nextCell.becomeHead();
		snakeCells.add(nextCell);
	}
	...
	protected BoardCell getSouthNeighbor(BoardCell cell) {
		int c = cell.getColumn();
		int r = cell.getRow();
		if (r < Preferences.NUM_CELLS_TALL - 1) {
            return boardCells2D[r + 1][c];
        }
        return null;
	}
	...
	protected BoardCell getEastNeighbor(BoardCell cell) {
		int c = cell.getColumn();
		int r = cell.getRow();
		if (r < Preferences.NUM_CELLS_WIDE - 1) {
            return boardCells2D[r][c + 1];
        }
        return null;
	}
	...
	protected BoardCell getNextCellInDir() {
		if (! (currentMode == SnakeMode.AI_MODE)){
			switch (currentMode){
				case GOING_NORTH:
	...
	public void reverseSnake() {
		...
		Iterator<BoardCell> descendingIterator = snakeCells.descendingIterator();

		while (descendingIterator.hasNext()) {
			reversedSnakeCells.add(descendingIterator.next());
		}

		snakeCells =  reversedSnakeCells;

		getSnakeHead().becomeHead();

		if (getNorthNeighbor(getSnakeHead()) == getSnakeNeck()){
			setDirectionSouth();
		} else if (getSouthNeighbor(getSnakeHead()) == getSnakeNeck()) {
			setDirectionNorth();
		} else if (getEastNeighbor(getSnakeHead()) == getSnakeNeck()) {
			setDirectionWest();
		} else if (getWestNeighbor(getSnakeHead()) == getSnakeNeck()){
			setDirectionEast();
		} 
	...
	protected BoardCell getNextCellFromBFS() {
		...
		BoardCell currentNode = cellsToSearch.peek();

		// search!
		while (!cellsToSearch.isEmpty()){
			BoardCell nextCell = getFirstCellInPath(currentNode);
			return nextCell;
		}
		for (int i=0;i<4;i++){
			// check NSEW neighbors of node 
			BoardCell currentNeighbor = getNeighbors(currentNode)[i];
			if (!currentNeighbor.inSearchListAlready() && !currentNeighbor.isWall() &&!currentNeighbor.isBody()){
				cellsToSearch.add(currentNeighbor);
				currentNeighbor.setAddedToSearchList();
	...
	private BoardCell getFirstCellInPath(BoardCell start) {
		/* HW10 Part 6 */
		BoardCell currentNode = start;
		while (currentNode.getParent() != this.getSnakeHead()){
			//backtrack recursively thru parents
